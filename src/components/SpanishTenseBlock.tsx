	...
	return (
		<>
			<tr className="text-[.8rem] verbBlock">
				...
				<td><span className={`${tenseClass} cursor-pointer select-none hover:underline`} onClick={() => handleToggleShowInfos('yo')} style={{ fontWeight: showInfos.yo ? 'bold' : 'normal' }}>{sv.conj.indicative[tenseIdCode].yo}</span><sup className="hasValue">{exampleCountObject[SpanishExampleManager.buildCountIdCode(sv.spanish, tenseIdCode, "yo")]}</sup><sup className="zero">0</sup></td>
