			...
			{examples.map(example => {
				...
				return (
					<div key={example.dpodId} className="mt-3">
						...
						<div style={{backgroundColor: exampleIsLearned ? 'red' : ''}} onClick={() => handleExampleToggle(example)} className={`border rounded-t-md py-1 px-2 bg-slate-200 italic text-slate-800 cursor-pointer border-r-slate-400 border-l-slate-400 border-t-slate-400 ${exampleIsLearned ? 'bg-green-400' : ''}`}>{example.english}</div>
						{example.isOpen && (
