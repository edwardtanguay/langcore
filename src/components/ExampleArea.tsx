			...
			{examples.map(example => {
				return (
					<div key={example.dpodId} className="mt-3">
						...
						{example.isOpen && (
							<div className="border rounded-b-md py-1 px-2 bg-slate-300 font-bold border-r-slate-400 border-l-slate-400 border-b-slate-400">{example.dutch}<PiSpeakerHighFill onClick={() => handleGoToAudioSite(example)} className="cursor-pointer inline-block ml-1" /><MdOutlineCheckBoxOutlineBlank onClick={() => handleGoToAudioSite(example)} className="cursor-pointer inline-block ml-1" /></div>
						)}
					</div>
				)
			})}
